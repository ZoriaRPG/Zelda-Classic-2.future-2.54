...
#define ZC_VERSION 25400 //Version ID for ZScript
#define VERSION_BUILD       32                              //build number of this version
#define VERSION_BETA       52                             //build number of this version
#define VERSION_LEAD       0                              //build number of this version
#define IS_BETA             1                               //is this a beta?
#define DATE_STR            "20th January, 2017"
...
#define V_FFSCRIPT         7
#define V_FFSCRIPT_2502 6 //The version for 2.50.2
#define V_SFX              6
...
#define MAXSFX 255 	//The highest SFX ID
...
#define MAXFFCS			 32
#define MAX_WIPE_TYPES 6 //Number of visual wipe effects, not including warp visuals. 
#define MAX_WARP_ANIMATIONS 6 //none, blackout, wipe, wave, zap, scroll
...
enum
{
	...
	//hole biscuits, there is already a frozen palette, so that I can add bool frozen to 
	//Someone thought to do this?! I'm shocked, truly shocked. 
};
...
enum
{ //Get rid of hardcoded decorations in favur of user-defined sprir=tes? -Z
...
enum                                                        // value matters because of current quest file data
{
    ...
    iLast, //icTrowel, icSomaria, icEther, icQuake, icBombos, icElectric, 
    //icScript1, icScript2, icSwordZ3
    iMax=256
};

// Shield projectile blocking
#define shROCK		0x001 //000000001b
#define shARROW		0x002 //000000010b
#define shBRANG		0x004 //000000100b
#define shFIREBALL	0x008 //000001000b
#define shSWORD		0x010 //000010000b
#define shMAGIC		0x020 //001000000b
#define shFLAME		0x040 //010000000b
#define shSCRIPT	0x080 //010000000b
#define shFIREBALL2	0x100 //100000000b // Boss fireball, not ewFireball2
//Script types, ice, lightning? -Z
...
enum
{
    ...
    wRefBeam, wStomp,
    lwMax, //wIce, wSound // -Z: sound + defence split == digdogger, sound + one hit kill == pols voice -Z
...
enum
{
    ...
    edefQUAKE,
    edefLAST, edefETHER, edefBOMBOS, edefPOT, edefTHROWNROCK, edefELECTRIC
};

#define edefSCRIPTDEFS_MAX 9
...
//Let'ssee if we need a packfile change for this, too. 
enum
{
	...
	edCHINKL10, //If damage is less than 10
	ed2x, //Double damage
	ed3x, //Triple Damage
	ed4x, //4x damage
	edHEAL, //recover the weapon damage in HP
	edTRIGGERSECRETS, //Triggers screen secrets.
...
#define ITEM_FLAG5     0x1000
    ...
    byte playsound;
    word collect_script;
    byte exp[10];                                             // not used - !!ZoriaRPG use for attributes
    long initiald[8];
    byte initiala[2];
    byte wpn;
    byte wpn2;
...
#define guy_bhit        0x00000001 //needs to be available to zscript. -Z
#define guy_invisible   0x00000002 //needs to be available to zscript. -Z
#define guy_neverret    0x00000004 //needs to be available to zscript. -Z
#define guy_doesntcount 0x00000008 //needs to be available to zscript. -Z

#define guy_fadeflicker 0x00000010 //needs to be available to zscript. -Z
#define guy_fadeinstant 0x00000020 //needs to be available to zscript. -Z
/*
...
#define guy_bkshield    0x10000000 // Shield can't be broken //needs to be available to zscript. -Z
//#define guy_mirror      0x20000000 // Shield is mirrored //needs to be available to zscript. -Z
//#define weak_L3brang    0x40000000

#define lens_only       0x80000000 //needs to be available to zscript. -Z

#define guy_flashing    0x00000001 //needs to be available to zscript. -Z
...
#define guy_blinking    0x00004000 //needs to be available to zscript. -Z
#define guy_transparent 0x00008000 //needs to be available to zscript. -Z
...
#define ffOVERLAY       0x00000001 //needs to be available to zscript. -Z
...
struct guydata
{
    ...
    short  dp, wdp, weapon; //needs to be available to zscript. -Z
    ...
    short  bgsfx, bosspal, extend; //needs to be available to zscript. -Z
    byte defense[edefLAST];
    
    //  short  startx, starty;
    //  short  foo1,foo2,foo3,foo4,foo5,foo6;
    byte  hitsfx, deadsfx; //needs to be available to zscript. -Z
...
struct mapscr
{
    ...
    word door_combo_set;
    byte warpreturnx[4];  //settilewarpreturn()?
    byte warpreturny[4];
    word warpreturnc;
    byte stairx;
    byte stairy;
    ...
    byte tilewarpscr[4];
    byte exitdir;
    word enemy[10]; //this is the enemy list. Needs to be r/w in ZScript. -Z
    byte pattern;
    byte sidewarptype[4];
    ...
    //  byte layeryspeed[6];
    //  byte layerydelay[6];
    byte layeropacity[6]; //should be available to zscript, I suppose.-Z
    word timedwarptics;
    byte nextmap;
    byte nextscr;
...
{ //Secret types
    ...
    sSCRIPT1, sSCRIPT2, sSCRIPT3, sSCRIPT4, sSCRIPT5, sSCRIPT6, sSCRIPT7,
	sSCRIPT8, sSCRIPT9, sSCRIPT10
};
...
struct comboclass
{
	//combo flags are here. -Z
	//the vars for pots/lift objectsd would need to go here. 
	//these values are what we need to be able to read-write from zscript. -Z
    char  name[64];                       //  name
...
extern comboclass *ffscriptcomboclass;

//These settings at least, should be available to zscript. We will need to add some things to this
//for new combo types, and for user defined combo values for the combo and flag editor. 
struct newcombo
{
    word tile;
    byte flip;
...
extern newcombo *ffscriptcombos;
...
struct zquestheader
{
    char  id_str[31];
    //32
    short zelda_version; //why do we lack guys version and ffscript version?
    word  internal;
    byte  quest_number;
    byte  old_rules[2];
    byte  old_map_count;
    ...
    byte  old_rules2[2];
    char  old_options;
    char  version[9]; //Quest version set by the questmaker. 
    //73
    char  title[65];
    char  author[65];
    //byte  padding;
...
#define MSGC_GOTOIFGLOBAL 3 // 3 args (Goto if Screen->D)
...
#define MSGC_SETSCRD	18 //2 args (register,value) !Dimentio!
#define MSGC_SETSCRD2	19 //3 args (register,value) !ZoriaRPG
#define MSGC_SFX	20 // 1 arg
#define MSGC_MIDI	21 // 1 arg
#define MSGC_NAME	22 // 0 args, disabled
#define MSGC_SETSCRD3	23 //4 args (register,value) !ZoriaRPG

#define MSGC_NEWLINE	25 // 0 args
#define MSGC_WARP	26 // 2 args (dmap,screen) !Dimentio!
#define MSGC_WARP2	27 // 3 args (dmap,screen,return) !Dimentio!

#define MSGC_WARP3	28 // 4 args (dmap,screen,return,sound)  !Dimentio!
#define MSGC_WARP4	29 // 4 args (dmap,screen,return,visual)  !Dimentio!
#define MSGC_GOTOIFYN   30 // 0 args, disabled (intended to be 'Goto if Yes/No').
		//This could be implemented on Ex1/Ex2 or L/R. 
...
//Above 30 are ASCII codes. -!Dimentio, can't use those. 
//I need to make an exception for 128 and above. 
/*
//30s are Warps
#define MSGC_GOTOIFYN   30 // 0 args, disabled
#define MSGC_WARP	31 // 2 args (dmap,screen) !Dimentio!
#define MSGC_WARP2	32 // 3 args (dmap,screen,return) !Dimentio!

#define MSGC_WARP3	33 // 4 args (dmap,screen,return,sound)  !Dimentio!
#define MSGC_WARP4	34 // 4 args (dmap,screen,return,visual)  !Dimentio!

//40s are Screen->D
#define MSGC_SETSCRD	40 //2 args (register,value) !Dimentio!
#define MSGC_SETSCRD2	41 //3 args (register,value) !ZoriaRPG
#define MSGC_SETSCRD3	42 //4 args (register,value) !ZoriaRPG
*/
...
struct MsgStr
{
    ...
    word tile;
    byte cset;
    bool trans; //oh, why isn't this implemented? At least it was reserved. -Z
    byte font;
    short x;
...
struct dmap
{
    ...
    //16
    char name[21];
    char title[21]; //these need font, tile, and bool trans, shadow
    char intro[73];
    //byte padding;
    //132
    ...
    //byte padding;
    //204
    byte disableditems[iMax]; //Wait, weapons greater than custom 20 can't be disabled?
    // 460
    long flags;
};
...
#define dmfSCRIPT6			0x10000 //Not yet implemented -Z
#define dmfSCRIPT7 			0x20000
#define dmfSCRIPT8 			0x40000
#define dmfSCRIPT9 			0x80000
#define dmfSCRIPT10 			0x100000

#define OLDMAXCOMBOALIASES 256
#define MAXCOMBOALIASES 2048 //4096 has been requested.
...
struct zcolors
{
    ...
    byte foo2[256];
    //296 bytes
    // byte fadeanim_colour; //default is black
};
...
struct gamedata
{
    ...
    //! These map values need to be available to ZScript. -Z
    ...
    byte get_throttle();
    void set_throttle(byte t);
    void change_throttle(short t);
