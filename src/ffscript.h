...
enum ASM_DEFINE
{
    ...
    GETFFCSCRIPT,         //0x00E8
   // GETDMAPSCREENDOOR,	//0x00E9
   // SETDMAPSCREENDOOR,	//0x00EA
   // GETDMAPSCREENSTATE,	//0x00EB
   // SETDMAPSCREENSTATE,	//0x00EC
   // GAMETHROTTLE, //0x00E9
    BITMAPEXR,	//0x00E9
    QUAD2R, //0x00EA
    //DRAWBITMAPEXR, //0x00EA
    WAVYIN, //0x00EB
    WAVYOUT, //0x00EC
    ZAPIN, //0x00ED
    ZAPOUT, //0x00EF
...
/* ..sorry, forgot about these ...for noAw. -Gleeok
      CALCSPLINE,           //0x00
      COLLISIONRECT,  ?      //0x00
      COLLISIONBOX,   ?      //0x00
      SETBITMAPBUFFER,
...
//#define LWPNMISC(n)        ((0x101D)+(n)) //0 spare, we now use all 32
#define EWPNMISCD            0x102D
//#define EWPNMISC(n)        ((0x102E)+(n)) //0 spare, we now use all 32
#define NPCMISCD             0x103E
//#define NPCMISC(n)         ((0x103F)+(n)) //0 spare, we now use all 32
#define ITEMMISCD            0x104F
//#define ITEMMISC(n)        ((0x1050)+(n)) //0 spare, we now use all 32
...
//#define FFINITD(n)		   ((0x108C)+(n)) //8 spare -: USed these -Z
#define LINKMISCD            0x1094
//#define LINKMISC(n)        ((0x1095)+(n)) //0 spare, we now use all 32
