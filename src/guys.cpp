...
{	//Check hitbox offsets, too. -Z
    return (abs(int( (LinkX()+Link.getXOfs()) )-x)<d && abs(int( (LinkY()+Link.getYOfs()) )-y)<d);
}
...
// 1: takehit returns 1
// -1: do damage
int enemy::scriptdefend(int wpnId, int *power,  int edef)
{
    if(shieldCanBlock)
    {
        switch(scriptdefense[edef])
    ...
    */
    ...
    //Level needs to be stored in the weapon itself, read from itemclass, for this to work.
    //!ZoriaRPG
...
int enemy::defenditemclass(int wpnId, int *power)
{
    ...
    case wScript1:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF1);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript2:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF2);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript3:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF3);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript4:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF4);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript5:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF5);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript6:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF6);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript7:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF7);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript8:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF8);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript9:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF9);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
    
    case wScript10:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power,  scriptDEF10);
	    else def = defend(wpnId, power,  edefSCRIPT);
        break;
...
int enemy::takehit(weapon *w)
{
    ...
    //&& !w->flag3
    //flag3 is 'Piercing', so we do not reverse its direction if it pierces. -Z
    else
        wpnDir = w->dir;
...
{
    int nx=LinkX()+Link.getXOfs(); //Calculate with hitbox offsets. -Z
    int ny=LinkY()+Link.getYOfs();
...
//    case right: nx+=((d->misc1==0)?32:48); if(nx-LinkX()<32) nx+=((d->misc1==0)?16:0); break;
    ...
    case down:
        ny+=((dmisc1==0||dmisc1==2)?32:48);
        //plus hitbox offsets =Z
        if(ny-LinkY()+Link.getYOfs()<32) ny+=((dmisc1==0||dmisc1==2)?16:0);
    ...
    case right:
        nx+=((dmisc1==0||dmisc1==2)?32:48);
        //plus hitbox offsets =Z
        if(nx-(LinkX()+Link.getXOfs())<32) nx+=((dmisc1==0||dmisc1==2)?16:0);
    ...
    if(d2>=left) //plus hitbox offsets =Z
        if(m_walkflag(LinkX()+Link.getXOfs(),LinkY()+Link.getYOfs(),spw_halfstep)||m_walkflag(LinkX()+Link.getXOfs(),LinkY()+Link.getYOfs()-8,spw_halfstep))                         /*none*/
            return false;
    ...
    if(misc==0) //inside wall, ready to spawn?
    {
        if(frame-wallm_load_clk>80 && clk>=0)
        {
            ...
            if(wall>0)
            {
                ...
                switch(dir)
                {
                case up: //plus hitbox offsets =Z
                    y=LinkY()+Link.getYOfs()+48-(wallm_cnt&1)*12;
                    flip=wall&1;
                    break;
                ...
                case left:
                    x=LinkX()+Link.getXOfs()+48-(wallm_cnt&1)*12;
                    flip=(wall==up?2:0)+1;
                    break;
...
bool eKeese::animate(int index)
{
    ...
    else if(get_bit(quest_rules,qr_ENEMIESZAXIS) && !(tmpscr->flags7&fSIDEVIEW))
    {
	    ...
	    //plus hitbox offsets =Z
        z+=int(step*zc_max(0,(distance(x,y,LinkX()+Link.getXOfs(),LinkY()+Link.getYOfs())-128)/10));
    }
...
int GuyHit(int tx,int ty,int tz,int txsz,int tysz,int tzsz)
{
	...
	if ( Link.getExtend() >= MIN_LINK_EXTEND ) {
		//tx = Link.getX() + Link.getHXOfs();
		//ty = Link.getY() + Link.getHYOfs();
		tx += Link.getHXOfs();
		ty += Link.getHYOfs();
		//ty -= Link.getHYSz();
		//ty += Link.getHYOfs();
		...
		//txsz = ( Link.getX() + Link.getHXSz() + Link.getHXOfs() );
		txsz = (Link.getHXSz()) + Link.getHXOfs();
		tysz = (Link.getHYSz()) + Link.getHYOfs();
		//What's the best way to set this one so that it starts at Link's lowest edge?
		//Just make the scripter use offsets?
		//tysz = ( Link.getY() + Link.getHYSz() + Link.getHYOfs() );
