...
//Stop the subscreen from falling?
...
//Sound and visuals during pitwarps?

int LinkClass::getPitWarpSound(){
	return pitwarpsfx;
}

	
void LinkClass::setPitWarpSound(int sound){
	pitwarpsfx = sound;
}

int LinkClass::getPitWarpEffect(){
	return pitwarpeffect;
}

void LinkClass::setPitWarpEffect(int fx){
	pitwarpeffect = fx;
}

fix LinkClass::getLinkHitZHeight(){
	return lzheight;
}

void LinkClass::setLinkHitZHeight(int z){
	lzheight = z;
}

//This is the boolean to toggle if they play.
//Toggle if pitwarps play sounds temporarily or perpetually?
//For now, the setting is temporary, and it resets to off every frame.
void LinkClass::setPlayPitWarpSound(bool v)
{
    playpitwarpsfx = v;
	
}

bool LinkClass::getPlayPitWarpSound()
{
	return playpitwarpsfx; 	
}

void LinkClass::setIsPitWarp(bool v)
{
    ispit = v;
	
}

bool LinkClass::getIsPitWarp()
{
	return ispit; 	
}
...
/*
void LinkClass::setExtend(int value)
{
	int v = vbound(value,0,4);
	for ( int q = 0; q <= 4; q++ ) {
		walkspr[q][spr_extend] = v;
		floatspr[q][spr_extend] = v;
		swimspr[q][spr_extend] = v;
		divespr[q][spr_extend] = v;
		slashspr[q][spr_extend] = v; 
		jumpspr[q][spr_extend] = v; 
		chargespr[q][spr_extend] = v; 
		stabspr[q][spr_extend] = v;
		poundspr[q][spr_extend] = v; 
	}
	castingspr[spr_extend] = value; 
	holdspr[spr_landhold][spr_hold1][spr_extend] = v; 
	holdspr[spr_landhold][spr_hold2][spr_extend] = v; 
	holdspr[spr_waterhold][spr_hold1][spr_extend] = v; 
	holdspr[spr_waterhold][spr_hold2][spr_extend] = v; 
}
...
int LinkClass::getExtend()
{
	...
	switch(action){
		...
		return walkspr[getDir()][spr_extend];	
}
*/
...
void LinkClass::setExtend(int value)
{
	if ( value < 0 ) { 
		...
		*/
		...
		switch(getSprite(action)){
			case 0: //none
			case 1: //walking
				walkspr[getDir()][spr_extend] = v;
				break;
			case 2: 
				floatspr[getDir()][spr_extend] = v;
				break;
			case 3: 
				swimspr[getDir()][spr_extend] = v;
				break;
			case 4:
				divespr[getDir()][spr_extend] = v;
				break;
			case 5:
				slashspr[getDir()][spr_extend] = v;
				break;
			case 6:
				jumpspr[getDir()][spr_extend] = v;
				break;
			case 7:
				chargespr[getDir()][spr_extend] = v;
				break;
			case 8:
				stabspr[getDir()][spr_extend] = v;
				break;
			case 9:
				poundspr[getDir()][spr_extend] = v;
				break;
			case 10:
				castingspr[spr_extend] = v;
				break;
			case 11:
				holdspr[spr_landhold][spr_hold1][spr_extend] = v;
				break;
...
int LinkClass::getExtend()
{
	switch(getSprite(action)){
		...
		case 1: //walking
			return walkspr[getDir()][spr_extend];	
		case 2: 
			return floatspr[getDir()][spr_extend];
		case 3: 
			return swimspr[getDir()][spr_extend];
		case 4:
			return divespr[getDir()][spr_extend];
		case 5:
			return slashspr[getDir()][spr_extend];
		case 6:
			return jumpspr[getDir()][spr_extend];
		case 7:
			return chargespr[getDir()][spr_extend];
		case 8:
			return stabspr[getDir()][spr_extend];
		case 9:
			return poundspr[getDir()][spr_extend];
		case 10:
			return castingspr[spr_extend];
		case 11:
			return holdspr[spr_landhold][spr_hold1][spr_extend];
...
void LinkClass::setSCCWarp(bool v)
{
    zstringwarp = v;
	
}

bool LinkClass::getSCCWarp()
{
	return zstringwarp; 	
}

    
int LinkClass::getSCCWarpSound()
{
    return sccwarpsound;
}

void LinkClass::setSCCWarpSound(int sound)
{
    sccwarpsound=vbound(sound,0,MAXSFX);
	
}

int LinkClass::getSCCWarpFX()
{
    return sccwarpfx;
}

void LinkClass::setSCCWarpFX(int fx)
{
    sccwarpfx=vbound(fx,0,7);
	
}

int LinkClass::getSCCWarpReturn()
{
    return sccwarpretsq;
}

void LinkClass::setSCCWarpReturn(int sq)
{
    sccwarpretsq=vbound(sq,0,3);
...
void LinkClass::setWarpSound(int sfx)
{
    warpsound=vbound(sfx,0,MAXSFX);
}

int LinkClass::getWarpSound()
{
    return warpsound;
}

void LinkClass::ClearSCCWarpValues(){
	setSCCWarp(false);
	setSCCWarpSound(0);
	setSCCWarpFX(0);
	setSCCWarpReturn(0);
...
//For tile warps and Link->Warp
void LinkClass::setWarpEffect(int fx)
{
    warpeffect=fx;
}

//For tile warps and Link->Warp
int LinkClass::getWarpEffect()
{
    return warpeffect;
}
...
//Play sound and visuals during sidewarps?
//Link->SideWarpSound
int LinkClass::getSideWarpSound(){
	return sidewarpsfx;
}
	
void LinkClass::setSideWarpSound(int sound){
	sidewarpsfx = sound;
}

//Link->SideWarpEffect
int LinkClass::getSideWarpEffect(){
	return sidewarpeffect;
}
void LinkClass::setSideWarpEffect(int fx){
	sidewarpeffect = fx;
}


//Toggle if sidewarps play sounds temporarily or perpetually?
//For now, the setting is static, once the user sets it, until they change it. 
...
bool LinkClass::getPlayLinkWarpSound()
{
	return playwarpsfx; 	
}
...
int LinkClass::getLinkSwimSpeed(){
	return zinit.link_swim_speed;
}

void LinkClass::setLinkSwimSpeed(int s){
	zinit.link_swim_speed = s;
}

int LinkClass::getLinkAnimationStyle(){
	return zinit.linkanimationstyle;
}
void LinkClass::setLinkAnimationStyle(int style){
	zinit.linkanimationstyle = style;
}

int LinkClass::getLinkWalkAnimSpeed(){
	return link_animation_speed;
}
void LinkClass::setLinkWalkAnimSpeed(int s){
	link_animation_speed = s;
}
...
void LinkClass::init()
{
    ...
    ilswim=true;
    walkable=false;
	preventsubscreenfalling = false; 
	hysz = DEFAULT_LINK_HYSZ; //Set up the default hitsizes so that if extend is set to >= 3 they don't
	hxsz = DEFAULT_LINK_HXSZ; //default at 0
	hxofs = 0;
	hyofs = 0;
    ...
    z=fall=0;
    hzsz = DEFAULT_LINK_HZSZ; // So that flying peahats can still hit him.
    ...
    sccwarpsound = 0;
    zstringwarp = false;
    playwarpsfx = false; //play a sound during sidewarps
    playpitwarpsfx = false; 
    isscriptedwarp = false;
    warpeffect = 0;
...
void LinkClass::draw_under(BITMAP* dest)
{
    ...
    if(action==rafting && c_raft >-1)
    {
        ...
        {
		//use drawx and drawy offsets for this sort of thing. 
...
int LinkClass::weaponattacklevel()
{
    int level = directWpn>-1 ? itemsbuf[directWpn].fam_type : (current_item_level(attack==wWand ? itype_wand : attack==wHammer ? itype_hammer : itype_sword));
...
void LinkClass::draw(BITMAP* dest)
{
    ...
    oxofs=xofs; //why are these used here? (I did not set these.) -Z
    oyofs=yofs; //Shouldn;t this be his draw offsets?
    ...
    {	
	//if ( getCanLinkFlicker() ) { //ZQuest thing to decide of Link can flicker or flash at all. 
		//Nothing to do with whichhe does, only if he cqan do either. 
        ...
        */
                ...
                || (attack==wHammer && attackclk<=30))
        {
            if(!invisible)
            {
                ...
                {
			//if ( (frame&1) ) //|| !getCanLinkFlicker() )
			//if ( getCanLinkFlicker() )
				 masked_draw(dest);
                }
            }
...
int LinkClass::EwpnHit()
{
    for(int i=0; i<Ewpns.Count(); i++)
    {
	    //this function needs to check the hit size and offsets -Z
        if( (getExtend() >= MIN_LINK_EXTEND Ewpns.spr(i)->hit(x+getXOfs()+7,y+getYOfs()+7,z,(Link.getHXSz()) + Link.getHXOfs(),(Link.getHYSz()) + Link.getHYOfs(),1)) || 
		( getExtend() < MIN_LINK_EXTEND && Ewpns.spr(i)->hit(x+7,y+7,z,2,2,1)) )
        {
...
void LinkClass::checkhit()
{
    ...
    if(hclk>=40 && action==gothit)
    {
        if(((ladderx+laddery) && ((hitdir&2)==ladderdir))||(!(ladderx+laddery)))
        {
            for(int i=0; i<4; i++)
            {
                switch(hitdir)
                {
                case up: //needs hitbox size
                    if( (getExtend() < MIN_LINK_EXTEND && (hit_walkflag(x,y+(bigHitbox?-1:7),2)||(int(x)&7?hit_walkflag(x+16,y+(bigHitbox?-1:7),1):0))) ||
			 (getExtend() >= MIN_LINK_EXTEND && (hit_walkflag(x+getXOfs(),y+getYOfs()+(bigHitbox?-1:7)2)||((int(x)+getXOfs())&7?hit_walkflag(x+getXOfs()+getHXSz(),getYOfs()+getY()+(bigHitbox?-1:7),1):0)))   
			action=none;
                    else --y;
                    
                    break;
                    
                case down: //needs hitbox size
                    if( (getExtend < MIN_LINK_EXTEND && (hit_walkflag(x,y+16,2)||(int(x)&7?hit_walkflag(x+16,y+16,1):0)) ) ||
			( getExtend()>= MIN_LINK_EXTEND && (hit_walkflag(x+XOfs(),y+16,2)||((int(x)+getXOfs())&7?hit_walkflag(x+16+getXOfs(),y+16+getYOfs(),1):0)) ) )
			action=none;
                    else ++y;
                    
                    break;
                    
                case left: //needs hitbox size
                    if( (getExtend < MIN_LINK_EXTEND && (hit_walkflag(x-1,y+(bigHitbox?0:8),1)||hit_walkflag(x-1,y+8,1)||(int(y)&7?hit_walkflag(x-1,y+16,1):0)) )  ||
			    (getExtend >= MIN_LINK_EXTEND && (hit_walkflag(x-1+getXOfs(),y+getYOfs()+(bigHitbox?0:8),1)||hit_walkflag(x+getXOfs()-1,y+getYOfs()+8,1)||((int(y)+getYOfs())&7?hit_walkflag(x+getXOfs()-1,y+getYOfs()+16,1):0))  ) )			    
		    action=none;
                    else --x;
                    
                    break;
                    
                case right: //Needs hitbox size -Z I believe that the 8s and 16s should be the hitbox size, but I;m not certain if the 8s should be hitbox size -8
                    if( (getExtend < MIN_LINK_EXTEND && (hit_walkflag(x+16,y+(bigHitbox?0:8),1)||hit_walkflag(x+16,y+8,1)||(int(y)&7?hit_walkflag(x+16,y+16,1):0)) ) ||
			(getExtend >= MIN_LINK_EXTEND && (hit_walkflag(x+getXOfs()+16,y+getYOfs()+(bigHitbox?0:8),1)||hit_walkflag(x+getXOfs()+16,y+getYOfs()+8,1)||((int(y)+getYOfs())&7?hit_walkflag(x+getXOfs()+16,y+getYOfs()16,1):0)) )     
		    action=none;
                    else ++x;
    ...
    for(int i=0; i<Lwpns.Count(); i++)
    {
        ...
        { //Add an includive AND here to check for getLinksFireHirtsLink()set by script. !ZoriaRPG
...
killweapon:
        ...
        { //Stick an includive AND here as && getLinkBombsHirtLink() for setting this by script. !ZoriaRPG
...
bool LinkClass::animate(int)
{
    ...
    if(isSideview())  // Sideview gravity
    {
        ...
        else if(fall <= (int)zinit.terminalv)
        {
                    ...
                    && fall < 0)
            {
                ...
                // ... maybe on spikes
		    if ( getExtend() < MIN_LINK_EXTEND ) checkdamagecombos(x+4, x+12, y-1, y-1);
		    if ( getExtend() >= MIN_LINK_EXTEND )
			checkdamagecombos(getX()+getXOfs()+4, getX()+getXOfs()+(getHXSZ()-2), getY()+getYOfs()-1, getY()+getYOfs()-1);
            }
    ...
    //oooooh boy. Link's offsets for the ladder will be a true joy. -Z
    if(diagonalMovement)
    {
        if(ladderx+laddery)
        {
    ...
    if(rSbtn()) //If Link presses the start button, and is on a save tile, or is not on a save tile...
    {
        int tmp_subscr_clk = frame;
        
        switch(lsave)
        {
        case 0: //Drop the active subscreen? -Z
...
int selectWpn_new(int type, int startpos, int forbiddenpos)
{
	...
	//We could ass more buttons to this... -Z
