		...
		if(masked) {	//stretched and masked
			...
			if ( rot != 0 ) { //if rotated
				switch(mode){
					...
					case 16: 
						//lit
					//rotate_sprite_lit(BITMAP *bmp, BITMAP *sprite, int x, int y, fixed angle);
					break;
			...
			if ( rot != 0 ) { //if rotated
				switch(mode){
					case 1: 
						//transparent
					//rotate_sprite_trans(BITMAP *bmp, BITMAP *sprite,int x, int y, fixed angle);
					stretch_blit(sourceBitmap, subBmp, sx, sy, sw, sh, 0, 0, dw, dh);
					break;
...
/* Dec2016 Polygon Template - Use this one...
...
void do_primitives(BITMAP *targetBitmap, int type, mapscr *, int xoff, int yoff)
{
    ...
    for(int i(0); i < numDrawCommandsToProcess; ++i)
    {
	 ...
	 case QUAD2R:
        {
            do_bmpdrawquadr(bmp, sdci, xoffset, yoffset);
        }
        break;
