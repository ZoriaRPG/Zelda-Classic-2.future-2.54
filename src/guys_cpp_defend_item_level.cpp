...
bool enemy::scriptcandamage(int power, int edef)
{
    switch(scriptdefense[edef])
    {
    case edSTUNORCHINK:
    case edSTUNORIGNORE:
    case edSTUNONLY:
...
// 1: takehit returns 1
// -1: do damage
int enemy::defend(int wpnId, int *power, int *level, int edef)
{
    if(shieldCanBlock)
    {
        switch(defense[edef])
...
// 1: takehit returns 1
// -1: do damage
int enemy::scriptdefend(int wpnId, int *power, int *level, int edef)
{
    if(shieldCanBlock)
    {
        switch(scriptdefense[edef])
...
int enemy::defenditemclass(int wpnId, int *power, int *level)
{
    int def=-1;
    ...
    case wBrang:
        def = defend(wpnId, power, level, edefBRANG);
        break;
        
    case wHookshot:
        def = defend(wpnId, power, level, edefHOOKSHOT);
        break;
        
        // Anyway...
    case wBomb:
        def = defend(wpnId, power, level, edefBOMB);
        break;
        
    case wSBomb:
        def = defend(wpnId, power, level, edefSBOMB);
        break;
        
    case wArrow:
        def = defend(wpnId, power, level, edefARROW);
        break;
        
    case wFire:
        def = defend(wpnId, power, level, edefFIRE);
        break;
        
    case wWand:
        def = defend(wpnId, power, level, edefWAND);
        break;
        
    case wMagic:
        def = defend(wpnId, power, level, edefMAGIC);
        break;
        
    case wHammer:
        def = defend(wpnId, power, level, edefHAMMER);
        break;
        
    case wSword:
        def = defend(wpnId, power, level, edefSWORD);
        break;
        
    case wBeam:
        def = defend(wpnId, power, level, edefBEAM);
        break;
        
    case wRefBeam:
        def = defend(wpnId, power, level, edefREFBEAM);
        break;
        
    case wRefMagic:
        def = defend(wpnId, power, level, edefREFMAGIC);
        break;
        
    case wRefFireball:
        def = defend(wpnId, power, level, edefREFBALL);
        break;
        
    case wRefRock:
        def = defend(wpnId, power, level, edefREFROCK);
        break;
        
    case wStomp:
        def = defend(wpnId, power, level, edefSTOMP);
        break;
        
    case wCByrna:
        def = defend(wpnId, power, level, edefBYRNA);
        break;
        
	
    case wScript1:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF1);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript2:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF2);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript3:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF3);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript4:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF4);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript5:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF5);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript6:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF6);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript7:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF7);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript8:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF8);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript9:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF9);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
    
    case wScript10:
	    if(QHeader.zelda_version > 0x250) def = scriptdefend(wpnId, power, level, scriptDEF10);
	    else def = defend(wpnId, power, level, edefSCRIPT);
        break;
...
int enemy::takehit(weapon *w)
{
    ...
    case wBrang:
    {
        int def = defend(wpnId, &power, &fam_type, edefBRANG);
    ...
    case wHookshot:
    {
        int def = defend(wpnId, &power, &fam_type, edefHOOKSHOT);
...
fsparkle:
    ...
    {
        int def = defenditemclass(wpnId, &power, &fam_type);
...
int ePatra::defend(int wpnId, int *power, int *level, int edef)
{
    int ret = enemy::defend(wpnId, power, level, edef);
...
int ePatraBS::defend(int wpnId, int *power, int *level, int edef)
{
    int ret = enemy::defend(wpnId, power, level, edef);
...
bool parsemsgcode()
{
    ...
    */
	...
	Link.setSCCWarpReturn(0); //No ret square arg, so use A (to be safe)
	Link.setSCCWarpFX(Link.getWarpEffect()); //Use the effect set by scripts, or nothing. 
	Link.setSCCWarpSound(Link.getWarpSound()); //Use whatever is set by scripts, or nothing. 
	if ( canwarp ){
		//Link.setWarpReturnSquare(vbound(destsq, 0, 3));
		Z_message("SCC WARP trying to warp Link to dmap, screen, warpdest\n", dmap, screen);
		
		Link.setSCCWarp(true);
	}
	return true;
      }
	...
	if ( canwarp ){
		Link.setSCCWarpReturn(vbound(destsq, 0, 3));
		Z_message("SCC WARP trying to warp Link to dmap, screen, warpdest\n", dmap, screen, destsq);
	...
	return true;
      ...
      case MSGC_WARP3: // !Dimentio!
    {	//dmap, screen, return, sound
	    bool canwarp = true;
        int dmap = grab_next_argument();
	if(dmap<0 || dmap>=MAXDMAPS)
		canwarp = false;
	...
	Link.setSCCWarpFX(Link.getWarpEffect()); //Use the effect set by scripts, or nothing. 
	Link.setSCCWarpReturn(vbound(destsq,0,3));
	Link.setSCCWarpSound(vbound(sfx,0,MAXSFX));
	tmpscr->sidewarpdmap[destsq] = dmap;
	tmpscr->sidewarpscr[destsq]  = screen;
	tmpscr->sidewarptype[destsq] = wtIWARP;
	...
	return true;
      ...
      case MSGC_WARP4: // !Dimentio!
    {	//dmap, screen, sound, effect
	    bool canwarp = true;
        int dmap = grab_next_argument();
	if(dmap<0 || dmap>=MAXDMAPS)
		canwarp = false;
	...
	if(DMaps[dmap].map*MAPSCRS+DMaps[dmap].xoff+screen>=TheMaps.size())
        canwarp = false;
	Link.setSCCWarpFX(vbound(fx,0,7));
	Link.setSCCWarpReturn(vbound(destsq,0,3));
	Link.setSCCWarpSound(vbound(sfx,0,MAXSFX));
	tmpscr->sidewarpdmap[destsq] = dmap;
	tmpscr->sidewarpscr[destsq]  = screen;
	...
	game->screen_d[di2][index] = value*10000;
	return true;
    }
    
    case MSGC_SETSCRD2: // screen, reg, value
    {
	int scr = grab_next_argument();
	int index = grab_next_argument();
	int value = grab_next_argument();
	int di2 = scr + ((get_currdmap())<<7);
    ...
    case MSGC_SETSCRD3: // dmap, screen, reg, value
    {
	
	int dmp = (grab_next_argument()<<7);
	int scr = grab_next_argument();
	int index = grab_next_argument();
	int value = grab_next_argument();
	int di2 = (dmp+scr);
    ...
    /*
    case MSGC_GETSCRD2: // screen, reg : GetScreenD
    {
	int index = grab_next_argument();
	int value = grab_next_argument();
	int di2 = (ri->d[0])/10000 + ((get_currdmap())<<7));
	...
	set_screen_d((, ri->d[1]/10000, value);
	
	return true;
    }
	
    case MSGC_GETSCRD3: // dmap, screen, reg GetDMapScreenD
    {
	int index = grab_next_argument();
	int value = grab_next_argument();
	int di2 = ((ri->d[1]/10000 + ((ri->d[0]/10000)<<7));
	...
	set_screen_d(, ri->d[2]/10000, value);
