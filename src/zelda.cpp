...
fix  LinkX()
{
    return Link.getX(); //+Link.getXOfs();
}
fix  LinkY()
{
    return Link.getY(); //+Link.getYOfs;
}
fix  LinkZ()
{
    return Link.getZ();
...
//Load the quest
    ...
    //Set the QRs to the ffscript array: -Z
    setRuleFF_Init();
    ...
    Link.setEntryPoints(LinkX(),LinkY()); //Don;t use his hitbox offsets here, yet. -Z    
...
int cont_game()
{
	...
	//! Here, we want to read the quest rules from the ZScript array in the future. 
	
    //  introclk=intropos=msgclk=msgpos=dmapmsgclk=0;
    didpit=false;
	
	//We might need to set some of the Link vars here that we haven;t cleared. -Z
...
void do_magic_casting()
{
    ...
    if(magicitem==-2) //Link->Action = LA_CASTING set from script
    {
	if((magiccastclk++)==0){
	...
	if((magiccastclk++)>=226)
	{
	    magicitem=-1;
	    magiccastclk=0;
		linktile(&Link.tile, &Link.flip, &Link.extend, ls_walk, Link.getDir(), zinit.linkanimationstyle);
	    //Link.setAction(none)
		return;
	}
	    /*
	...
	if(castnext)
        ...
        if((magiccastclk++)>=226)
        {
            magicitem=-1;
            magiccastclk=0;
		linktile(&Link.tile, &Link.flip, &Link.extend, 1, Link.getDir(), zinit.linkanimationstyle);
	    //Link.setAction(none)
		return;
        }
	*/
...
int main(int argc, char* argv[])
{
	...
	Z_title("Zelda Classic %s Alpha (ID %d)",VERSION_LEAD, VERSION_BETA);
        break;
        
    case 1:
        Z_title("Zelda Classic %s Beta (ID: %d)",VerStr(ZELDA_VERSION), VERSION_BETA);
	Z_title("Zelda Classic %s Beta (ID %d)",VERSION_LEAD, VERSION_BETA);
        break;
