...
bool LinkClass::animate(int)
{
    ...
    if(isSideview())  // Sideview gravity
    {
        ...
        else if(fall <= (int)zinit.terminalv)
        {
            ...
            if((_walkflag(x+getXOfs()+4,y+getYOfs()-(bigHitbox?9:1),0)
                || (y<=(bigHitbox?9:1) &&
        ...
        for(int i=-1; i<2; i++)
        {
            if(dir==up)
            {
                if((combobuf[MAPCOMBO2(i,x,y-7)].type==cHSGRAB)||
                        (_walkflag(x+getXOfs()+2,y+getYOfs()+4,1) && !ishookshottable(int(x),int(y+4))))
                {
                    use_hookshot=false;
                }
            }
...
void LinkClass::movelink()
{
    ...
    if(action==walking) //still walking
    {
        ...
        {
            if(dir==up&&yoff)
            { //added x/y offsets to solidity checks here -Z 14-Jan-17
                info = walkflag(x+getXOfs(),y+getYOfs()+(bigHitbox?0:8)-int(lsteps[int(y)&7]),2,up);
                if(blockmoving)
                    info = info || walkflagMBlock(x+8+getXOfs(),y+getYOFs()+(bigHitbox?0:8)-int(lsteps[int(y)&7]));
                execute(info);
            ...
            if(dir==down&&yoff)
            { //added x/y offsets to solidity checks here -Z 14-Jan-17
                info = walkflag(x+getXOfs(),y+getYOfs()+15+int(lsteps[int(y)&7]),2,down);
                if(blockmoving)
                    info = info || walkflagMBlock(x+getXOfs()+8,y+getYOfs()+15+int(lsteps[int(y)&7]));
                execute(info);
            ...
            if(dir==left&&xoff)
            {  //added x/y offsets to solidity checks here -Z 14-Jan-17
                info = walkflag(x+getXOfs()-int(lsteps[int(x)&7]),y+getYOfs()+(bigHitbox?0:8),1,left) || walkflag(x+getXOfs()-int(lsteps[int(x)&7]),y+getYOfs()+8,1,left);
                execute(info);
            ...
            if(dir==right&&xoff)
            {  //added x/y offsets to solidity checks here -Z 14-Jan-17
                info = walkflag(x+getXOfs()+15+int(lsteps[int(x)&7]),y+getYOfs()+(bigHitbox?0:8),1,right) || walkflag(x+getXOfs()+15+int(lsteps[int(x)&7]),y+getYOfs()+8,1,right);
                execute(info);
    ...
    if(diagonalMovement)
    {
        ...
        if(DrunkUp()&&(holddir==-1||holddir==up))
        {
            ...
            else
            {
                ...
                {  //added x/y offsets to solidity checks here -Z 14-Jan-17
                    info = walkflag(x+getXOfs(),y+getYOfs()+(bigHitbox?0:8)-z3step,2,up);
                    
                    if(int(x) & 7)  //added x/y offsets to solidity checks here -Z 14-Jan-17
                        info = info || walkflag(x+getXOfs()+16,y+getYOfs()+(bigHitbox?0:8)-z3step,1,up);
                    else if(blockmoving)  //added x/y offsets to solidity checks here -Z 14-Jan-17
                        info = info || walkflagMBlock(x+getXOfs()+16, y+getYOfs()+(bigHitbox?0:8)-z3step);
                    ...
                    if(info.isUnwalkable())
                    {
                        if(z3step==2)
                        {
                            z3step=1;  //added x/y offsets to solidity checks here -Z 14-Jan-17
                            info = walkflag(x+getXOfs(),y+getYOfs()+(bigHitbox?0:8)-z3step,2,up);
                            
                            if(int(x)&7) //added x/y offsets to solidity checks here -Z 14-Jan-17
                                info = info || walkflag(x+getXOfs()+16,y+getYOfs()+(bigHitbox?0:8)-z3step,1,up);
                            else if(blockmoving) //added x/y offsets to solidity checks here -Z 14-Jan-17
                                info = info || walkflagMBlock(x+getXOfs()+16, ygetYOfs()+bigHitbox?0:8)-z3step);
                ...
                else
                {
                    if(s==left)
                    {  //added x/y offsets to solidity checks here -Z 14-Jan-17
                        info = (walkflag(x+getXOfs()-1,y+getYOfs()+(bigHitbox?0:8),1,left)||walkflag(x-1,y+15,1,left));
                        execute(info);
